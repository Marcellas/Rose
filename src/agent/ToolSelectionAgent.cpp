    ...
    std::string ToolSelectionAgent::buildSystemPrompt() const
    {
        ...
        prompt
            ...
            << "For read_text_file specifically, use it only when the contents of one "
               ...
               "request. Require an explicit absolute file path. If the exact file is not "
               "known yet, use list_directory first when appropriate. Do not use "
               "read_text_file for PDFs, images, executables, archives, or other binary "
               "formats. Do not read unrelated files merely because they are nearby.\n\n"
